    ...
    if (!empty($_POST['buy'])) {
        ...
        if ($insert) {
            ...
            for ($i = 0; $i <= count($buy_ids); $i++) {
                $updateQuery = $db->query("UPDATE livestock SET quantity = quantity - 1 WHERE id = '$id[$i]'");
            }

            echo '<div class="alert alert-success alert-dismissable">
              <a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
               <strong>Discount made successfully <i class="fa fa-check"></i></strong>
          </div>';
            ...
            echo '<div class="alert alert-danger alert-dismissable">
              <a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
               <strong>Error creating discount. Please try again <i class="fa fa-times"></i></strong>
          </div>';
        }
    }
...
?>
 ...
 <div class="w3-row">
    <form method="POST" action="discount.php" id="buyForm">
        <h2>Manage Discount</h2>
        <a href="discount.php" style="margin-left: 10px" class="btn btn-sm btn-danger pull-right">Cancel discount</a>
        <a href="javascript:;" data-toggle="modal" data-target="#myModal" class="btn btn-sm btn-primary pull-right">Complete discount</a><br><br>
            ...
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        ...
                        <h4 class="modal-title" id="myModalLabel">Complete discount</h4>
                    </div>
    ...
    <div class="table-responsive">
 	    <table class="table table-hover table-striped" id="table">
 		    ...
 		    <tbody>
            <?php
                $all_pig = $db->query("SELECT * FROM livestock WHERE reorder = 1 ORDER BY id DESC");
...
<script>
    function submit_reorder() {
        alert('Discount will be submitted!');
        $('#buyForm').submit();
    }
</script>